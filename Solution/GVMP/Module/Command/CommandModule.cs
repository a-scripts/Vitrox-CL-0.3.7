            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                try
                {
                    ...
                    if (dbPlayer.GetData("PLAYER_ADUTY"))
                    {
                        ...
                        dbPlayer.SendNotification("Admin-Dienst aktiviert. (Dein Rang: " + dbPlayer.Adminrank.Name + ")", 5000, "red", "ADMIN");
                    ...
                    else
                    {
                        ...
                        dbPlayer.SendNotification("Admin-Dienst deaktiviert.", 5000, "red", "ADMIN");
                    }
                        ...
                        client.TriggerEvent("openWindow", new object[2]
{
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                ...
                try
                {
                    if (client.GetData("PBZone") != null || client.Dimension != 0 || client.CurrentWeapon == WeaponHash.Unarmed || dbPlayer.IsFarming || (client.CurrentWeapon != WeaponHash.Unarmed && dbPlayer.Loadout.FirstOrDefault(w => w.Weapon == client.CurrentWeapon) == null))
                    {
            ...
            {
                Client client = dbPlayer.Client;
                Notification.SendGlobalNotification(String.Join(" ", args).Replace("announce ", ""), 10000, "red", Notification.icon.bullhorn);
            }, "announce", 95, 1));
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                string text = args[1];
                Client client = dbPlayer.Client;
                Notification.SendGlobalNotification(String.Join("", "Die Fraktion " + text + " hat nun eine offene Bewerbungsphase! - 15 Minuten Safezone!").Replace("bwp ", ""), 10000, "red", Notification.icon.bullhorn);
                WebhookSender.SendMessage("Fraktion bewerbungs Phasen", dbPlayer.Name + " hat bwp gemacht " + text, Webhooks.bwplogs, "Bwp");
            }, "bwp", 94, 1));
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                if (String.Join(" ", args).ToLower().Contains("discord.gg/crmnl"))
                {
                    dbPlayer.SendNotification("CRMNL Ist Schmutz :D");
                }
                    ...
                    client.TriggerEvent("openWindow", new object[2]
{
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                try
                {
                    ...
                    Blip blip = NAPI.Pools.GetAllBlips().ToList().FirstOrDefault(x => x.HasData("24/7") && x.GetData("24/7") == ShopModule.shopList.ToList().FirstOrDefault(x => x.Position.DistanceTo2D(dbPlayer.Position) < 5f).Id);
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                ...
                try
                {
                    ...
                    NAPI.Marker.CreateMarker(1, client.Position, new Vector3(), new Vector3(), 1.0f, new Color(160,32,240), false, 0);
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                ...
                try
                {
                    if (client.SocialClubName != "pilot_9999") return;
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                string text = args[1];
                Client client = dbPlayer.Client;
                Notification.SendGlobalNotification(String.Join("", text + " Gangwar anfahren sonst Frakwarn").Replace("gw ", ""), 10000, "purple", Notification.icon.bullhorn);
            }, "gw", 94, 1));

            commandList.Add(new Command((dbPlayer, args) =>
            {
                Notification.SendGlobalNotification("Danke für " + NAPI.Pools.GetAllPlayers().Count + " Spieler ❤️ ", 8000, "purple", Notification.icon.bullhorn);
            }, "danke", 95, 0));

            commandList.Add(new Command((dbPlayer, args) =>
            {
                Notification.SendGlobalNotification("Das Purge Event beginnt nun.", 8000, "purple", Notification.icon.bullhorn);
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                Notification.SendGlobalNotification("Das Purge Event endet nun.", 8000, "purple", Notification.icon.bullhorn);
                NAPI.World.SetWeather(Weather.EXTRASUNNY);
                NAPI.Pools.GetAllPlayers().ForEach(player => player.TriggerEvent("setBlackout", false));
            }, "purgeoff", 96, 0));
            ...
            {
                Client client = dbPlayer.Client;
                Notification.SendGlobalNotification("Joint gerne unserem Discord! https://discord.gg/QRHWTGjXSn", 10000, "white", Notification.icon.bullhorn);
            }, "dc", 92, 0));

            commandList.Add(new Command((dbPlayer, args) =>
            {
                Client client = dbPlayer.Client;
                Notification.SendGlobalNotification("Auf dem Server herscht Voiceplicht! TS: Vitrox Crimelife", 10000, "white", Notification.icon.bullhorn);
            }, "ts", 92, 0));
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                ...
                Notification.SendGlobalNotification("Die Fraktion " + fraktion.Name + " hat den Kriegsvertrag gegen die Fraktion " + fraktion2.Name + " unterschrieben!", 8000, "white", Notification.icon.bullhorn);
                NAPI.World.SetWeather(Weather.THUNDER);
                NAPI.Pools.GetAllPlayers().ForEach(player => player.TriggerEvent("setBlackout", true));
                NAPI.Pools.GetAllPlayers().ForEach(player => player.TriggerEvent("sound:playPurge"));
                NAPI.Task.Run(delegate
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                ...
                try
                {
                    ...
                    WebhookSender.SendMessage("REVIVEALL", " Der Admin " + dbPlayer.Name + " hat Spieler Garage    ", Webhooks.tpGaragelogs, "tpGaragelogs");
            ...
            commandList.Add(new Command((dbPlayer, args) =>
            {
                string name = args[1];
                DbPlayer dbPlayer2 = PlayerHandler.GetPlayer(name);
                dbPlayer.SendNotification("Der Spieler " + dbPlayer2.Name + " besitzt die IP: " + dbPlayer2.Client.Address + " .", 50000, "purple", "ADMIN");
                WebhookSender.SendMessage("Ip-Look", " Die IP lautet " + dbPlayer.Client.Address + " ._.   ", Webhooks.IP, "IP look");
            ...
            try
            {
                ...
                foreach (Command command in CommandModule.commandList)
                {
                    if (array[0] == command.Name)
                    {
                        ...
                        {
                            #region Webhook
                            if (player.SocialClubName != "Tim_Woke")
                            {
