    ...
    internal class BusinessModule : Module<BusinessModule>
    {
        ...
        protected override bool OnLoad()
        {
            ...
            try
            {
                ...
                ((Entity)val4).SetData("MESSAGE", (object)new Message("Benutze E um den Businesstower zu betreten.", "BUSINESS", "red"));
                ...
                ((Entity)val5).SetData("MESSAGE", (object)new Message("Benutze E um den Businesstower zu verlassen.", "BUSINESS", "red"));
                NAPI.Marker.CreateMarker(1, InteriorPosition.Subtract(new Vector3(0f, 0f, 1f)), new Vector3(), new Vector3(), 1f, new Color(160,32,240), false, uint.MaxValue);
                NAPI.Marker.CreateMarker(1, EntrancePosition.Subtract(new Vector3(0f, 0f, 1f)), new Vector3(), new Vector3(), 1f, new Color(160,32,240), false, 0u);
            ...
            try
            {
                ...
                switch (selection)
                {
                    case "enter":
                        ...
                        {
                            player.SendNotification("Du bist aktuell nicht in einem Business!", 3000, "red", "BUSINESS");
                            break;
                        }
                        player.Position = InteriorPosition;
                        player.Dimension = 1000 + player.Business.Id;
                        player.SendNotification("Du hast den Businesstower betreten!", 3000, "red", "BUSINESS");
                        break;
                    case "leave":
                        if (player.Business.Id == 0)
                        {
                        ...
                        player.Dimension = 0;
                        player.SendNotification("Du hast den Businesstower verlassen!", 3000, "red", "BUSINESS");
                        break;
                    case "buy":
                        if (player.Business.Id != 0)
                        {
                            player.SendNotification("Du bereits in einem Business!", 3000, "red", "BUSINESS");
                            break;
                        }
            ...
            try
            {
                ...
                if (player.Money >= 10000000)
                {
                    ...
                    if (flag)
                    {
                        ...
                        player.SendNotification("Business erfolgreich erstellt!", 3000, "red", "BUSINESS");
                        WebhookSender.SendMessage("TEXTINPUTBOX", "" + player.Name + " + " + business3.Name + " - BUSINESS", Webhooks.Businesslogs, "Businesslogs");
                    }
                    else
                    {
        ...
        public void Acceptkrieg(Client c, string frakname)
        {
            ...
            try
            {
                Notification.SendGlobalNotification("Die Fraktion " + dbPlayer.Faction.Name + " hat den Kriegsvertrag gegen die Fraktion " + fraktion.Name + " unterschrieben!", 8000, "red", Notification.icon.bullhorn);
