	...
	internal class BusinessApp : Script
	{
		...
		public void saveBusinessMOTD(Client c, string motd)
		{
			try
			{
				...
				if (player != null && player.IsValid(ignorelogin: true) && !((Entity)(object)player.Client == (Entity)null) && player.CanInteractAntiFlood(1) && player.Business.Id != 0 && player.Businessrank >= 1)
				{
					if (motd.Length > 32)
					{
						player.SendNotification("MOTD zu lang, maximal 32 Zeichen!", 3000, "red", "Business");
						return;
					}
					...
					player.SendNotification("MOTD geändert!", 3000, "red", player.Business.Name);
				}
			}
			catch (Exception ex)
			{
		...
		public void leaveBusiness(Client c)
		{
			try
			{
				...
				foreach (DbPlayer businessPlayer in player.Business.GetBusinessPlayers())
				{
					businessPlayer.SendNotification("Der Spieler " + c.Name + " hat das Business verlassen.", 3000, "red", businessPlayer.Business.Name);
				}
				player.Business = BusinessModule.getBusinessById(0);
				player.Businessrank = 0;
				player.RefreshData(player);
				player.SendNotification("Du hast das Business verlassen.", 3000, "red", "Business");
			}
			catch (Exception ex)
			{
			...
			try
			{
				...
				if (player != null && player.IsValid(ignorelogin: true) && !((Entity)(object)player.Client == (Entity)null) && player.CanInteractAntiFlood(1) && player.Business.Id != 0 && player.Businessrank >= 1)
				{
					...
					else if (player2.Business.Id == player.Business.Id && player2.Businessrank <= player.Businessrank)
					{
						...
						player.SendNotification("Du hast den Spieler aktualisiert.", 3000, "red", player.Business.Name);
					}
				}
			}
			catch (Exception ex)
		...
		public void kickBusinessMember(Client c, int id)
		{
			try
			{
				...
				else
				{
					...
					{
						player.SendNotification("Das kannst du nicht da der Spieler den/einen höheren Rang als du hat!", 3000, "red", player.Business.Name);
						return;			
					}
					...
					foreach (DbPlayer businessPlayer in player.Business.GetBusinessPlayers())
					{
						businessPlayer.SendNotification("Der Spieler " + player2.Name + " hat das Business verlassen.", 3000, "red", player.Business.Name);
					}
					player.SendNotification("Du hast den Spieler gekickt.", 3000, "red", player.Business.Name);
					return;
				}
			}
			catch (Exception ex)
		...
		public void addPlayerToBusiness(Client c, string name)
		{
			...
			try
			{
				...
				if (player.Businessrank > 0)
				{
					...
					{
						player.SendNotification("Der Spieler ist bereits in deinem Business.", 3000, "red", "BUSINESS");
					}
					else if (player2.Business.Id == 0)
					{
						...
						player.SendNotification("Du hast " + name + " eine Einladung gesendet.", 3000, "red", "BUSINESS");
					}
					else
					{
						player.SendNotification("Dieser Spieler ist bereits in einem Business.", 3000, "red", "BUSINESS");
					}
				}
				else
				{
					player.SendNotification("Du hast dazu keine Berechtigung.", 3000, "red", "BUSINESS");
				}
			}
			catch (Exception ex)
			{
		...
		public void acceptBusinessInvite(Client c, string bus, object unused)
		{
			...
			try
			{
				...
				{
					businessPlayer.SendNotification("Der Spieler " + c.Name + " hat das Business betreten.", 3000, "red", businessPlayer.Business.Name);
				}
				...
				player.SetAttribute("Businessrank", 0);
				player.SendNotification("Du bist dem Business " + businessById.Name + " beigetreten.", 3000, "red", "BUSINESS");
			}
			catch (Exception ex)
			{
